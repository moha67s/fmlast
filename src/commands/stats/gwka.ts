...
export default class GlobalWhoKnowsAlbumCommand extends BaseCommand {
    name = 'gwka';
    description = 'View who knows an album across the entire bot';
    aliases = ['gwkalbum', 'globalwhoknowsalbum'];

    slashData = new SlashCommandBuilder()
        .setName('gwka')
        .setDescription('View who knows an album across the entire bot')
        .addStringOption((opt: any) => 
            opt.setName('album')
                .setDescription('The album to check')
                .setRequired(false)
        )
        .addStringOption((opt: any) => 
            opt.setName('artist')
                .setDescription('The artist of the album')
                .setRequired(false)
        );
    ...
    async execute(interactionOrMessage: any, isSlash = false, args?: string[]): Promise<void> {
        ...
        if (!albumName) {
            const payload = new ComponentsV2().setAccent(0xff0000).addText('❌ Please specify an album or scrobble something first!').build();
        ...
        try {
            // 1. Resolve Artist ID and Album ID
            ...
            // Note: If artistName is not provided, we have to fall back to a string match.
            // FMBot gwka requires artist context usually. If no artist, we do string match on albumName.
            let knowers: any[] = [];
            let finalName = albumName;
            let finalArtistName = artistName;

            if (artistId) {
                const albumId = await IdResolutionService.getAlbumId(artistId, albumName);
                
                knowers = await (prisma.userAlbum as any).findMany({
                    where: { albumId },
                    include: { user: true },
                ...
                if (album) finalName = album.name;
                if (artist) finalArtistName = artist.name;
            } else {
                knowers = await (prisma.userAlbum as any).findMany({
                    where: { albumName: { equals: albumName, mode: 'insensitive' } },
                    include: { user: true },
                    orderBy: { playcount: 'desc' },
                    take: 100
                });
            ...
            if (finalArtistName) {
                try {
                    const meta = await TrackResolverService.resolveAlbum(finalArtistName, finalName);
