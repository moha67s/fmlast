...
export default class UnloveCommand extends BaseCommand {
    name = 'unlove';
    description = 'Unlove a track on Last.fm';
    aliases = [];

    slashData = new SlashCommandBuilder()
        .setName('unlove')
        .setDescription('Unlove a track on Last.fm')
        .addStringOption(opt => 
            opt.setName('track')
                .setDescription('The track name to unlove (defaults to current track)')
                .setRequired(false)
        )
        .addStringOption(opt => 
            opt.setName('artist')
    ...
    async execute(interactionOrMessage: any, isSlash = false, args?: string[]): Promise<void> {
        ...
        if (!dbUser || !dbUser.lastfmSessionKey) {
            const payload = new ComponentsV2().setAccent(0xff0000).addText('❌ You must be logged in to Last.fm to unlove tracks. Use `/login`.').build();
        ...
        if (!trackName || !artistName) {
            const errorPayload = new ComponentsV2().setAccent(0xff0000).addText('❌ Could not determine a track to unlove. Please provide it manually: `.unlove track | artist`').build();
        ...
        try {
            await LastFM.unlove(artistName, trackName, dbUser.lastfmSessionKey);
            const payload = new ComponentsV2()
                .setAccent(embedColor)
                .addText(`💔 Unloved **${trackName}** by **${artistName}** on Last.fm.`)
                .build();
            ...
            console.error(err);
            const payload = new ComponentsV2().setAccent(0xff0000).addText(`❌ Failed to unlove track: ${err.message}`).build();
            if (isSlash) await interactionOrMessage.editReply(payload);
            else await interactionOrMessage.channel.send(payload);
        }
    }
